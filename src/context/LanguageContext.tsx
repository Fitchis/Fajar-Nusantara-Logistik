...
const defaultLang: Lang = ((): Lang => {
  if (typeof navigator === 'undefined') return 'id';
  const navLang = (navigator as unknown as { language?: string; userLanguage?: string }).language ||
    (navigator as unknown as { language?: string; userLanguage?: string }).userLanguage ||
    'id';
...
export const LanguageProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  const [lang, setLangState] = useState<Lang>(() => {
    try {
      ...
      // if storage is not available, fall back to default
